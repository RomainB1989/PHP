...
CREATE TABLE IF NOT EXISTS orders(
   ...
   user_id INT NOT NULL,
   CONSTRAINT fk_order_location FOREIGN KEY(id_location) 
...
CREATE TABLE IF NOT EXISTS contain(
   ...
   CONSTRAINT fk_contain_product FOREIGN KEY(product_id) 
   REFERENCES products(product_id) ON DELETE CASCADE,
   CONSTRAINT fk_contain_order FOREIGN KEY(order_id) 
...
CREATE TABLE IF NOT EXISTS `show`(
   ...
   CONSTRAINT fk_show_product FOREIGN KEY(product_id) 
   REFERENCES Products(product_id) ON DELETE CASCADE,
   CONSTRAINT fk_show_image FOREIGN KEY(id_image) 
   REFERENCES image(id_image) ON DELETE CASCADE
);
CREATE TABLE IF NOT EXISTS possess(
   id_location INT NOT NULL,
   id_image INT NOT NULL,
   PRIMARY KEY(id_location, id_image),
   CONSTRAINT fk_possess_location FOREIGN KEY(id_location) 
   REFERENCES location(id_location) ON DELETE CASCADE,
   CONSTRAINT fk_possess_image FOREIGN KEY(id_image) 
   REFERENCES image(id_image) ON DELETE CASCADE
);
CREATE TABLE IF NOT EXISTS has(
   ...
   CONSTRAINT fk_has_article FOREIGN KEY(article_id) 
   REFERENCES Article(article_id) ON DELETE CASCADE,
   CONSTRAINT fk_has_image FOREIGN KEY(id_image) 
   REFERENCES image(id_image) ON DELETE CASCADE
);
